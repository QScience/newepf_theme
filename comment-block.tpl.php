...
$items = array();
  ...
  foreach (comment_get_recent($number) as $comment) {
    $items[] = l($comment->subject, 'comment/' . $comment->cid, array('fragment' => 'comment-' . $comment->cid)) . 
		'<div class="small_text_12">by '.($comment->uid==0?$comment->name:l($comment->name,'user/'.$comment->uid,array('attributes' => array ('class'=>array('username'))))) . t(' (@time)', array('@time' => format_date($comment->changed))) . '</div>';
  }
